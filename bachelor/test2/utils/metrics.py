...
def demand_accuracy(loader, batch_size, model, device, exp_dir=None):
    ...
    correct = 0
    with torch.no_grad():
        for modality1, modality2, labels in loader:
            modality1, modality2, labels = modality1.to(device), modality2.to(device), labels.to(device)

            outputs = model(modality1, modality2) # shape[batch_size *num_traffic *(N+1) *N]
...
def path_accuracy(loader, batch_size, model, device, exp_dir=None):
    ...
    return correct /(num_traffic *(batch_size *len(loader))) *100
...
def element_accuracy(loader, batch_size, model, device, exp_dir=None):
    ...
    return correct /(N *num_traffic *(batch_size *len(loader))) *100