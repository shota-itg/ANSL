...
def generate_static_graph(links) -> dict:
    """ 固定のリンクコストグラフを生成
    Args: 
...
def generate_graph(links) -> dict:
    ...
    for (u, v), link in links.items():
        ...
        if link.max_capacity == -1 or link.max_capacity <= link.used:
            weight_uv = float('inf')
            weight_vu = float('inf')
        else:
