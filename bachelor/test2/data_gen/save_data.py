...
## CSV保存
def save_data(lf_enable, num_traffic, max_failure, data_name, data_log, links_log, target_log, data_one_hot_log, target_one_hot_log):
    ...
    if lf_enable:
        if data_name == "train":
            save_traffic_log_path = os.path.join(
                ...
                lf_data_path_cfg["filename"]["lf_traffic_log_csv"].format(numt=num_traffic, k=max_failure, num=num_train_data)
            )
            save_data_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                topo_name, 
                lf_data_path_cfg["train_dir"], 
                lf_data_path_cfg["filename"]["lf_data_csv"].format(numt=num_traffic, k=max_failure, num=num_train_data)
            )
        else:
            save_traffic_log_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                ...
                lf_data_path_cfg["filename"]["lf_traffic_log_csv"].format(numt=num_traffic, k=max_failure, num=num_test_data)
            )
            save_data_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                topo_name, 
                lf_data_path_cfg["test_dir"], 
                lf_data_path_cfg["filename"]["lf_data_csv"].format(numt=num_traffic, k=max_failure, num=num_test_data)
            )
    else:
        if data_name == "train":
            save_traffic_log_path = os.path.join(
                ...
                data_path_cfg["filename"]["traffic_log_csv"].format(numt=num_traffic, num=num_train_data)
            )
            save_data_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                topo_name, 
                data_path_cfg["train_dir"], 
                data_path_cfg["filename"]["data_csv"].format(numt=num_traffic, num=num_train_data)
            )
        else:
            save_traffic_log_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                ...
                data_path_cfg["filename"]["traffic_log_csv"].format(numt=num_traffic, num=num_test_data)
            )
            save_data_path = os.path.join(
                config["paths"]["results"]["root_dir"], 
                topo_name, 
                data_path_cfg["test_dir"], 
                data_path_cfg["filename"]["data_csv"].format(numt=num_traffic, num=num_test_data)
            )
