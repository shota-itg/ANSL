# data_gen/network2.py

"""
トラフィック生成
    静的な経路設計
        任意の数のトラフィックが生成されることを前提にその経路を設計する
"""
...
def dijkstra(graph, start, end, nodes):
    ...
    while queue:
        ...
        if node == end:
            if not len(path) == len(nodes):
                path += [-1] * (len(nodes)-len(path))
            return path
        if node in visited:
            continue
...
if __name__ == "__main__":  # Python ファイルを直接実行したときだけこのブロックを実行するという意味 # 他のファイルからインポートされたときは実行されない
    ...
    def data_to_one_hot(src, dst, bw, num_nodes):
        one_hot = []
    ...
    while time.time() - start_time < duration:
        ...
        while not queue.empty():
            ...
            path = dijkstra(graph, src, dst, nodes)
            if not path:
                path = [-1] * len(nodes)
                data_log.append([src, dst, bw])
                target_log.append(path)
                continue
            ...
            # 失敗した通信は呼損
                ...
                data_log.append([src, dst, bw])
                target_log.append([-1, -1, -1, -1])

        time.sleep(0.5)
            
    for data in data_log:
        data_one_hot_log.append(data_to_one_hot(data[0], data[1], data[2], len(nodes)))

    for target in target_log:
        target_one_hot_log.append(target_to_one_hot(target, len(nodes)))
