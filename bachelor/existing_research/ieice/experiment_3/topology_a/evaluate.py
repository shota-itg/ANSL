...
if __name__ == "__main__":
    ...
    while queue_djk:
        ...
        max_load_link_list_djk += [max(links_djk_list)]
    ...
    max_djk = max(max_load_link_list_djk)
    min_djk = min(max_load_link_list_djk)
    average_djk = statistics.mean(max_load_link_list_djk)
    std_dev_djk = statistics.stdev(max_load_link_list_djk)
    std_err_djk = std_dev_djk / (len(max_load_link_list_djk) ** 0.5)
    median_djk = statistics.median(max_load_link_list_djk)
...
print(f'Average Time: {statistics.mean(time_djk)}')
...
print(f'Average Time: {statistics.mean(time_ex)}')
