...
labels_reshaped = labels.reshape(10000, 48, 5)
labels_argmax = np.argmax(labels_reshaped, 2)   # shape: [10000, 48]
...
class Net(nn.Module):
    ...
    def multi_nn(self, input_dim, hidden_dim=input_dim, output_dim, num_hidden_layer):
        layers = []
...
class MultiNet(nn.Module):
    ...
    def forward(self, x):
        shared = self.shared_layer(x)  # shape: [batch_size, hidden_dim]
        outputs = [block(shared) for block in self.output_blocks]  # list of [batch_size, output_dim]
        return torch.cat(outputs, dim=1)  # shape: [batch_size, num_nodes * output_dim]
    


class MultiSequentialNet(nn.Module):
    def __init__(self):
        super(MultiSequentialNet, self).__init__()
    ...
    def forward(self, inputs):
        ...
        return outputs  # 出力はリスト（output_0, output_1, ...）
...
def path_accuracy(loader, model, device):
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            outputs_argmax = np.argmax(outputs_reshaped, 2) # shape: [64, 48]
            outputs_reshaped = outputs_argmax.reshape(64, 12, 4)

            labels_reshaped = labels.reshape(64, 12, 4)

            correct += (outputs_reshaped == labels_reshaped).all(dim=2).sum().item()
            total += labels_reshaped.numel() // labels_reshaped.shape[2]
    model.train()
...
def objective(trial):
    ...
    for epoch in range(max_epoch):
        ...
        for each_batch in train_loader:
            # each_batch を入力値 x と目標値 t に分ける
            inputs, labels = each_batch
            
            inputs, labels = inputs.to(device), labels.to(device)

            outputs = net(inputs)   # shape: [64, 240]
            outputs_reshaped = outputs.reshape(64, 48, 5)
...
study.optimize(objective, n_trials=10)  # n_trials: 試行回数
...
print(f'outputs_softmax: \n{torch.softmax(outputs_reshaped, dim=1)}')   # デバック
