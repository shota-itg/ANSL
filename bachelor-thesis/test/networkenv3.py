...
if __name__ == "__main__":  # Python ファイルを直接実行したときだけこのブロックを実行するという意味 # 他のファイルからインポートされたときは実行されない
    ...
    queue = Queue() # 通信要求キュー
    data_log = []   # 空のリスト (list)
    data_one_hot_log = []   # 空のリスト (list)
    target_log = [] # 空のリスト (list)
    target_one_hot_log = [] # 空のリスト (list)
    traffic_log = []
    traffic_one_hot_log = []
        ...
        for _ in range(len(traffic_list)):
            ...
            if success:
                ...
                for link in link_path:
                    duration = bw / (link.max_capacity-link.used)
                    threading.Thread(target=release_later, args=(link, bw, duration)).start()
            ...
            time.sleep(random.uniform(0.001, 0.1))  # トラフィック生成間隔
