...
num_datasets = 1000 # ほしいデータセット数を設定
...
### Call クラス ###
class Call:
    def __init__(self, src, dst, bw, start_time, holding_time):
        self.src = src
        self.dst = dst
        self.bw = bw
        self.start_time = start_time
        self.holding_time = holding_time
        self.end_time = start_time + holding_time
        self.path = None
...
### Node クラス ###
class Node:
    def __init__(self, node_id, lambda_rate):
        self.node_id = node_id
        self.lambda_rate = lambda_rate
        self.next_call_time = 0

    def generate_call(self, current_time, nodes):
        inter_arrival = np.random.exponential(1 /self.lambda_rate)
        self.next_call_time = current_time +inter_arrival

        dst = random.choice([n for n in nodes if n != self.node_id])
        bw = random.choice(bandwidth_options)
        holding_time = np.random.exponential(180)

        return Call(self.node_id, dst, bw, self.next_call_time, holding_time)
...
def generate_graph(links):
    ...
    for (src, dst), link in links.items():  # links.items() は辞書 (dictionary) のすべてのキーと値のペアを取り出す  # 例）(0,1): Link(1000) → src=0, dst=1, link = Link(1000)
        ...
        """
        graph[src][dst] = link.used    # 使用量に応じて重み変化    # 二次元辞書（辞書の中に辞書）  # 1e-6: ゼロ除算を防ぐための微小値
        """
...
def dijkstra(graph, start, end):
    ...
    while queue:
        min_cost = queue[0][0]
        count = sum(1 for item in queue if item[0] == min_cost)
        if count == 0:
        ...
        if node == end:
            heapq.heappush(shortest, (cost, len(path)-1, node, path))
            # debug #print(f'')
            continue
        ...
        for neighbor, bandwidth in random.sample(list(graph.get(node, {}).items()), len(graph.get(node, {}))):    # neighbor: 隣接ノード  # graph.get(node, {}): 「 node の隣接ノード一覧」を取得 # .items(): 「隣接ノードと重み」のペアを取り出す
            if neighbor not in path:
                heapq.heappush(queue, (cost+bandwidth, len(path)-1, neighbor, path+[neighbor]))
...
def dijkstra(graph, start, end):
    ...
    while queue:
        ...
        # 隣接ノードの探索
        for step, (neighbor, bandwidth) in enumerate(random.sample(list(graph.get(node, {}).items()), len(graph.get(node, {})))):    # neighbor: 隣接ノード  # graph.get(node, {}): 「 node の隣接ノード一覧」を取得 # .items(): 「隣接ノードと重み」のペアを取り出す
            if neighbor not in path:
                heapq.heappush(queue, (cost+bandwidth, step, neighbor, path+[neighbor]))
...
def dijkstra(graph, start, end):
    ...
    cost, node, path = 0, start, [start]

    while node != end:
        # debug #
        print(f'cost={cost}, node={node}, path={path} <-- 探索開始')
        shortest = []
        ...
        # for neighbor, bandwidth in graph.get(node, {}).items():
            if neighbor not in path:
                heapq.heappush(shortest, (cost+bandwidth, len(path)-1, neighbor, path+[neighbor]))

                # debug #
                print(f'cost={cost+bandwidth}, node={neighbor}, path={path+[neighbor]} <-- 探索結果を追加')
...
if __name__ == "__main__":  # Python ファイルを直接実行したときだけこのブロックを実行するという意味 # 他のファイルからインポートされたときは実行されない
    ...
    while len(target_log) < num_datasets:
        ...
        for t in range(len(traffic_list)):
            ...
            # delay = random.uniform(8e-4, 5e-3)
            delay = 1 /bw
            ...
            if len(allocated_links) == len(link_path):
                ...
                def release_later(each_link, delay_time, bandwidth):
                    ...
                    time.sleep(delay_time *(len(each_link)-1))
                    # time.sleep(delay_time)
                ...
                """ 解放処理（def release_later():）
                for _ in range(int((delay_time *(len(path)-1) *1e-5) *1e+6)):
                    if stop_event.is_set():
                        return  # 外から止められたら終了
                    time.sleep(1e-6)

                for _ in range(len(path)-1):
                    time.sleep(delay_time) ########################################################### ここで調整 
                """


                """ 経路上のリンクを順に解放（スレッドだから順番が不安定）
    ...
    print(f'max_load_link_list: {max_load_link_list_ex}')
