...
class EarlyStopping:
    ...
    def __init__(self, patience=5, verbose=False, delta=0, path='results/checkpoint_model.pth'):
    ...
    def __call__(self, val_loss, model):
        ...
        if self.best_val_loss is None: # 1Epoch 目の処理
            ...
            print(f'Validation loss decreased ({self.val_loss_min:.12f} --> {val_loss:.12f}).')
            # self.save_checkpoint(val_loss, model) # 記録後にモデルを保存してスコアを表示
        elif val_loss < self.best_val_loss -self.delta:   # ベストスコアを更新した場合
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        self.val_loss_min = val_loss    # その時の loss を記録c