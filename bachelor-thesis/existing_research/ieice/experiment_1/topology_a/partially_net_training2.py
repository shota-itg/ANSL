...
labels_reshaped = labels.reshape(10000, NP_2, N, (N+1))   # shape: [len(dataset), P(N,2), N, (N+1)]
...
def element_accuracy(loader, model, device):
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            outputs_argmax = np.argmax(outputs_softmax, 3)  # shape: [128, 30, 6] <-- shape: [batch_size, P(N,2), N]
...
model = MultiSequentialNet().to(device)
criterion = nn.CrossEntropyLoss()
