...
### ネットワークの定義 ###
class FullyConnectedLayerNet(nn.Module):
    """
    ...
    def __init__(self):
        ...
        # 入力層
        layers.append(nn.Linear((N+1)*NP_2, (N+1)*NP_2*NP_2))
        layers.append(nn.ReLU())

        # 中間層
        for _ in range(num_hidden_layer):
            layers.append(nn.Linear((N+1)*NP_2*NP_2, (N+1)*NP_2*NP_2))
            layers.append(nn.ReLU())

        # 出力層
        layers.append(nn.Linear((N+1)*NP_2*NP_2, (N+1)*N*NP_2))
...
def lr_range_test(model, loader, criterion, device):
    optimizer = torch.optim.Adamax(model.parameters(), lr=1e-7)
...
model = FullyConnectedLayerNet().to(device)
...
for epoch in range(max_epoch):
    ...
    for inputs, labels in train_loader:
        ...
        outputs = model(inputs)  # shape: [batch_size, ((N+1)*N)*P(N,2)]
        outputs_reshaped = outputs.reshape(batch_size, NP_2, N, (N+1))    # reshape(batch_size, P(N,2), N, (N+1)) <--
...
plt.suptitle(f'H={num_hidden_layer}_BS={batch_size}_Initial lr={initial_lr} - Final lr={final_lr}')
plt.tight_layout()
plt.show()
