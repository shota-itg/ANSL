# evaluation/eval_link_failure_stateaware.py
    ...
    for _ in range(num_trials):
        ...
        for modality1, modality2, labels in testloader:
            ...
            with torch.no_grad():
                ...
                # debug #normal_outputs = model(modality1, modality2)
...
##############################################################################################################################
                ...
                for key in initial_load_dict:
                    if initial_load_dict[key] == -1:
                        current_capacity_dict[key] = -1
                        if key in current_adj:
                            current_adj.remove(key)
                ...
                if all_checker:
                    for key in current_load_dict:
                        ...
                        if current_capacity_dict[key] != -1:
                            modify_links_list += [current_load_dict[key].item()]

                    """
                    print("\nlinks_list: ", links_list)
                    print("current_capacity_dict:", current_capacity_dict)
                    print("initial_load_dict: ", initial_load_dict)
                    print("current_load_dict: ", current_load_dict)
                    print("len(modify_links_list): ", len(modify_links_list))
                    print("modify_links_list: ", modify_links_list)
                    print()                    
                    """


                    links_log.append(links_list)
                    mlu_log.append(max(modify_links_list))
                    std_log.append(statistics.stdev(modify_links_list))
                    if len(modify_links_list) <= 1 or statistics.mean(modify_links_list) == 0:
                        cv_log.append(0.0)
                    else:
                        cv_log.append(statistics.stdev(modify_links_list) /statistics.mean(modify_links_list))
    ...
    link_failure_stateaware_results.append((num_failure, max_success_rate *100, min_success_rate *100, average_success_rate *100, mlu_avg, mlu_std, mlu_min, mlu_max, std_avg, std_std, cv_avg, cv_std))

    print(f'[Number of Link Failure: {num_failure}] max={max_success_rate *100:.4f}, min={min_success_rate *100:.4f}, average={average_success_rate *100:.4f}, reachable_rate={reachable_rate *100:.4f}, congestion_rate={congestion_rate *100:.4f}')
...
def main(device, exp_dir=None):
    ...
    link_failure_results_log = []
    for num_failure in range(max_random_failure +1):
        ...
        link_failure_results = eval_link_failure(test_loader, inference_batch_size, model, device, config, hparam_cfg, num_failure)

        link_failure_results_log.append(link_failure_results)
    ...
    for link_failure_results in link_failure_results_log:
        for (fail_k, max_rate, min_rate, avg_rate, mlu_avg, mlu_std, mlu_min, mlu_max, std_avg, std_std, cv_avg, cv_std) in link_failure_results:
            target = overwrite_json["results"]["link_failure_stateaware_resilience"].setdefault(f"random_{fail_k}_link_failure", {})
            target["accuracy"] = {
                "max_success_rate": float(max_rate),
                "min_success_rate": float(min_rate),
                "average_success_rate": float(avg_rate)
            }
            target["load_balance_metrics"] = {
                "maximum_link_utilization": {
                    "average": float(mlu_avg), 
                    "standard_deviation": float(mlu_std), 
                    "min": float(mlu_min), 
                    "max": float(mlu_max)
                }, 
                "standard_deviation": {
                    "average": float(std_avg), 
                    "standard_deviation": float(std_std), 
                }, 
                "coefficient_of_variation": {
                    "average": float(cv_avg), 
                    "standard_deviation": float(cv_std), 
                }
            }
    ...
    with open(results_csv_path, "w", newline="") as f:
        ...
        for link_failure_results in link_failure_results_log:
            for (k, max_success_rate, min_success_rate, average_success_rate, _, _, _, _, _, _, _, _) in link_failure_results:
