...
#######################################################################################
    ...
    while queue:
        ...
        print()
        """        
    if no_path_checker is False or fukusou_sub_counter != 0:
        # 全リンクを解放
        for link in links.values():
            link.used = 0
        continue
...
if data_name == "train":
    if lf_enabled:
        data_json_path = os.path.join(
            ...
            topo_name, 
            lf_data_json_path_cfg["train_dir"], 
            lf_data_json_path_cfg["filename"]["lf_data_json"].format(numt=num_traffic, k=max_failure, num=num_data)
        )
    else:
        data_json_path = os.path.join(
            config["paths"]["results"]["root_dir"], 
            topo_name, 
            data_json_path_cfg["train_dir"], 
            data_json_path_cfg["filename"]["data_json"].format(numt=num_traffic, num=num_data)
        )
else:
    if lf_enabled:
        data_json_path = os.path.join(
            config["paths"]["results"]["root_dir"], 
            topo_name, 
            lf_data_json_path_cfg["test_dir"], 
            lf_data_json_path_cfg["filename"]["lf_data_json"].format(numt=num_traffic, k=max_failure, num=num_data)
        )
    else:
        data_json_path = os.path.join(
    ...
    "initial": {
        "load_balance_metrics": {
            "maximum_link_utilization": {
                "average": float(mlu_shortest_avg), 
                "standard_deviation": float(mlu_shortest_std), 
                "min": float(mlu_shortest_min), 
                "max": float(mlu_shortest_max)
            }, 
            "standard_deviation": {
                "average": float(std_shortest_avg), 
                "standard_deviation": float(std_shortest_std)
            }, 
            "coefficient_of_variation": {
                "average": float(cv_shortest_avg), 
                "standard_deviation": float(cv_shortest_std)
            }
        }        
    }, 
    "Dijkstra": {
        "load_balance_metrics": {
            "maximum_link_utilization": {
                "average": float(mlu_djk_avg), 
                "standard_deviation": float(mlu_djk_std), 
                "min": float(mlu_djk_min), 
                "max": float(mlu_djk_max)
            }, 
            "standard_deviation": {
                "average": float(std_djk_avg), 
                "standard_deviation": float(std_djk_std)
            }, 
            "coefficient_of_variation": {
                "average": float(cv_djk_avg), 
                "standard_deviation": float(cv_djk_std)
            }
        }        
    }
}
...
with open(data_json_path, "w") as f:
    json.dump(define_data_json, f, indent=4)


save_data(lf_enabled, num_traffic, max_failure, data_name, data_log, links_log, target_log, data_one_hot_log, target_one_hot_log)
if lf_enabled:
    if max_failure < num_failure:
        preprocess_data(data_name, num_traffic, lf_enabled, max_failure, num_data)
    else:
        preprocess_data(data_name, num_traffic, lf_enabled, num_failure, num_data)
else:
    preprocess_data(data_name, num_traffic, lf_enabled, max_failure, num_data)
