...
def main(device, exp_dir=None):
    ...
    if test_lf_enabled:
        test_modality1_pt = os.path.join(
            ...
            lf_datasets_path_cfg["filename"]["lf_modality1_pt"].format(numt=num_traffic, k=max_random_failure, num=num_test_data)
        )
        test_modality2_pt = os.path.join(
            config["paths"]["results"]["root_dir"], 
            topo_name, 
            lf_datasets_path_cfg["test_dir"], 
            lf_datasets_path_cfg["filename"]["lf_modality2_pt"].format(numt=num_traffic, k=max_random_failure, num=num_test_data)
        )
        test_labels_pt = os.path.join(
            config["paths"]["results"]["root_dir"], 
            topo_name, 
            lf_datasets_path_cfg["test_dir"], 
            lf_datasets_path_cfg["filename"]["lf_labels_pt"].format(numt=num_traffic, k=max_random_failure, num=num_test_data)
        )
    else:
        test_modality1_pt = os.path.join(
            config["paths"]["results"]["root_dir"], 
    ...
    with torch.no_grad():
        for modality1, modality2, labels in test_loader:
