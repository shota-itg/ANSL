...
def main():
    ...
    topo_root = os.path.join("experiments", topo_name, f'num_traffic_{num_traffic}', f'train_lf_{str(train_lf_enabled)}')
    ...
    try:
        ...
        exp_dir = os.path.join(topo_root, timestamp)
        ...
        final_train_loss = read_json.get("results", {}).get("train", {}).get("loss", {}).get("final_train_loss", 1)
        final_val_loss = read_json.get("results", {}).get("train", {}).get("loss", {}).get("final_val_loss", 1)
        ...
        """
        if 70 < final_train_path_accuracy and 60 < final_val_path_accuracy and 70 < test_path_accuracy:
            ...
            print()
            print("Running evaluation scripts: python3 -m evaluation.eval_link_failure_stateaware")
            subprocess.run([
                "python3", "-m", "evaluation.eval_link_failure_stateaware", 
                "--device", str(device), 
                "--exp_dir", exp_dir, 
            ])
            print()        
